...
| B7 Audit and operations | in_progress | [B7 local evidence](../ops/evidence/B7-audit-operations.md): local preparation complete; operator-owned self-pilot and conditional `SW` receipts recorded; no external effect yet; live durability unproven | Live Logpush/R2/alert-routing proof before the `SW` receipt expires |
