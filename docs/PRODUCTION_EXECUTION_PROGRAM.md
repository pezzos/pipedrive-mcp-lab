...
| B7 Audit and operations | in_progress | [B7 evidence](../ops/evidence/B7-audit-operations.md): hash-verified limited synthetic sandbox Logpush/R2 delivery, sampled byte integrity, one offline query, credential revocation, authority-linked old-job removal, historical cutover sequential gate 229/229, append-only email receipt/operator acknowledgement at the appshot share timestamp, and current post-ack sequential gate 237/237 | Alert blocker closed; obtain the exhaustive B7 sandbox validation packet and remaining non-backup live checks (including unproven Alexandre-only read access and actual-live-job-failure notification); production durability/routing is future scope outside B7 |
