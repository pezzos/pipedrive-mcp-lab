...
| B7 Audit and operations | in_progress | [B7 local evidence](../ops/evidence/B7-audit-operations.md): local preparation complete; conditional `SW` receipt recorded; no external effect; live durability unproven | Recorded pilot exception receipt, then live Logpush/R2/alert-routing proof before the `SW` receipt expires |
