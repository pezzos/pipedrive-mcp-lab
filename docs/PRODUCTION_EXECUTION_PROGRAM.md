...
| B7 Audit and operations | in_progress | [B7 evidence](../ops/evidence/B7-audit-operations.md): local preparation plus partial hash-verified sandbox R2/Logpush/alert configuration and one isolated synthetic heartbeat; no Logpush/R2 delivery proof | Execute authorized `DW` token cleanup, then trace, immutable receipt, queryability, and alert-ack proof work before `SW` expiry |
