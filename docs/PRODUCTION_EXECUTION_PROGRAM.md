...
| B7 Audit and operations | in_progress | [B7 evidence](../ops/evidence/B7-audit-operations.md): local preparation plus partial hash-verified sandbox R2/Logpush/alert configuration; destination-write connectivity only, no Worker trace delivery | Exact separate sandbox Worker deployment authority, then exact `DW` authority for token cleanup; then trace, receipt, queryability, and alert-ack proof work before `SW` expiry |
