# Claude Plugin And Desktop Extension Delivery

This package stages a Claude skills plugin and a Pipedrive MCP Desktop
Extension. The plugin contains Pipedrive-specific skills only; the `.mcpb`
extension provides the local MCP server and editable settings.
...
connector is required for those surfaces.
...
does not install the local connector there. Reliable Cowork, web, and mobile
availability requires a separately hosted remote MCP connector.
