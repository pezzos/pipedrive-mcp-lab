...
- Mailbox read/link tools when writes are enabled. Mailbox access may require an
  OAuth access token with the right Pipedrive scopes.
...
Build the server first, then configure the MCP host to run `node dist/server.js`
from this repository or from the unpacked private package.
...
| `PIPEDRIVE_ENABLE_WRITES` | `false` | Registers write and Mailbox tools when `true`. |
...
- Write and Mailbox tools are not registered unless `PIPEDRIVE_ENABLE_WRITES=true`.
